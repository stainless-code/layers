...
- **open** — push a layer onto a stack; returns `Promise<R>` resolved by `dismiss`, or rejected by `loadFn` / validation / `cancelAll`. The caller `await`s the user's decision.
