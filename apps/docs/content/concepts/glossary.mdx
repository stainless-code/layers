...
<Accordion>
  <AccordionItem title="Core entities" defaultOpen>
    ...
    - **`#dispatch`** — internal `LayerStack` choke point: every snapshot-changing mutation is labeled, applied, then `#flush`’d. Not a public API; unlocks an honest notify stream.
    - **StackNotifyEvent** — typed, JSON-safe record of one observable stack transition (`action` + `active`/`queued` views + optional projected `payload`). Owned and emitted by core; Devtools bridges it to TanStack `EventClient`.
    - **subscribeNotify** — `LayerClient` method: `(listener) => unsubscribe`; fans out `StackNotifyEvent`s. Escape hatch for headless tooling; zero cost when no listeners. Distinct from `subscribe` / `subscribeStacks` (snapshot / stack-created).
