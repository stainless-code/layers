...
<Accordion>
  ...
  <AccordionItem title="Scoping & caching">
    ...
    - **scope** — per-stack queueing on `StackOptions`: `scope: { strategy: "serial" | "parallel", onLoadError?: "block" | "advance" }` (`parallel` / `block` when omitted); `serial` = one occupying layer at a time, `parallel` = stack freely. `onLoadError` (`block` default / `advance`) — see Serial queues.
