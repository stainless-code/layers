...
:::note
Stars and npm downloads measure adoption, not engine depth. Layers is v0.2.2 and early — adoption lag is not an engine verdict.
...
- **Vue** — `jenesius-vue-modal` git-active (Jun 2026; npm last 2024-07); `kolirt/vue-modal` closest Vue analog (grouped stacks + async guards + nested groups, Reka UI integration); `vue-final-modal` stalled since 2024-09.
...
- **Angular** — `ngx-call` react-call port (low adoption); `@ngneat/dialog` source gone, still ~6.9k dl/wk — a vacuum; `@angular-libs/dialog` native `<dialog>` + plugin lifecycle (created 2026-05).
