...
Every adapter exposes the same primitives — client context, `useLayerClient`, options-bag `useStack({ stack, select, compare })`, **drive** handles (`useLayer` / `injectLayer` / `createLayer`), and **observe** hooks (`useLayerState` / `injectLayerState` / `createLayerState`) — and re-exports the full core. **Engine and primitive parity.** Ergonomic wrappers (`StackOutlet`, `createStackHook`, …) are sugar, not extra engine capability; they diverge by design (matrix below). **Angular and Svelte** ship compiler-free and render via imperative / primitive APIs instead of shipped components.
