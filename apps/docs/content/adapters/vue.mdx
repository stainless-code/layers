...
`StackOutlet` renders registered `component` entries. For scoped-slot-style subscriptions, use `StackSubscribe` or `useStack(stack, selector)` directly in `setup()`.
...
| Export | Role |
| ------ | ---- |
| `useStack(stackId?, selector?, compare?)` | `Readonly<Ref<T>>` of the selected stack slice |
| `useLayer(key, stackId?, compare?)` | `Readonly<Ref<LayerState \| null>>` for one active key |
