...
`useStack(stack, selector)` returns a readonly `Signal` — the idiomatic equivalent of `StackSubscribe`.
...
| `useLayerClient()` | Inject the app client (injection context required) |
| `useStack(stackId?, selector?, compare?)` | Readonly `Signal<T>` of the selected stack slice |
| `useLayer(key, stackId?, compare?)` | `Signal<LayerState \| null>` for one active key |

Both `useStack` and `useLayer` accept an optional leading `LayerClient` argument. Must run in an injection context.
