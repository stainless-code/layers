...
On a **serial** stack, that error layer occupies the lane until dismiss (`onLoadError: "block"`, the default). Set `onLoadError: "advance"` to remove it and drain the queue instead — see [Serial queues](/guides/serial-queues).
