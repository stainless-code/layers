...
A permitted user dismissal / `dismissAll` completes `open()` with `R` (including `undefined` for `R = void`) — not the return value of `dismiss` itself (`Promise<boolean>`). Narrow cancel with `isLayerCancelledError` — do not treat it as the user choosing a falsy response.
