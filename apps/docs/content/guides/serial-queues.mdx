...
By default (`onLoadError: "block"`), a rejecting `loadFn` leaves `phase: "error"` mounted and still occupying the lane — queued work waits, and further opens queue — until you dismiss it from error UI or `catch`.
