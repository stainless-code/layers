...
A rejecting `loadFn` leaves `phase: "error"` mounted. By default that layer still occupies the lane (`onLoadError: "block"`) — queued work waits, and further opens queue — until you dismiss it from error UI or `catch`.
