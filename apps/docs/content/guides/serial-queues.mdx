...
for (const step of steps) {
  void client.open(step); // only one active; rest queue // [!code highlight]
}
```
...
In adapters, observe queued layers with `useLayerQueuedState({ key, stack })` (per-key) or `useQueuedStack({ stack })` (whole stack) — both return `LayerState[]` for `phase: "queued"` rows. Wired handles expose the same slice as `.queued` on `useLayer(options)`.
