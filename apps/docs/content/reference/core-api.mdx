...
JSON-safe transition record for `subscribeNotify` (Devtools and headless inspectors). Distinct from `subscribe` snapshot identity — see [Devtools](/guides/devtools).

<AutoTypeTable path="../../packages/core/src/types.ts" name="StackNotifyAction" />
<AutoTypeTable path="../../packages/core/src/types.ts" name="LayerNotifyView" />
<AutoTypeTable path="../../packages/core/src/types.ts" name="StackNotifyEvent" />
...
System teardown (`cancelAll`, parent-dismiss child drain, group dispose, host disconnect) rejects `open()` with `LayerCancelledError`. A permitted user dismissal completes the layer's `open()` with `R` (`dismiss` returns `Promise<boolean>`; `dismissAll` returns `Promise<void>`). See [Error handling](/guides/error-handling).
