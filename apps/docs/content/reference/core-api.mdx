...
Symbols below are exported from `@stainless-code/layers` (`packages/core`). Adapters re-export the full core — install one adapter package and import everything from there.
...
class LayerStack {
  ...
  cancelQueued(key: LayerKey, response): boolean;
  settle(layer): void;
...
| `find` | Lookup by logical key (key signature) |
