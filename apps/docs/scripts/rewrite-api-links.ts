...
/** Match Blume heading ids: lowercase, spaces→`-`, strip non `[a-z0-9-]`. */
 ...
 * normalization once.
 *
 * Generated files are `.mdx`, so Blume base-prefixes every site-relative
 * body link at render time (verified) — we emit `/reference/api/...` (no
 * `deployment.base`, no extension) and let Blume handle the base.
 */
...
function normalizeAnchor(
  ...
  // TypeDoc `<a id>` member anchors aren't headings — Blume validate ignores
 ...
 * names modules by package path, and Blume does NOT base-prefix body links to
...
/**
 * Rename generated files whose slug contains a `.` to the dash-slug so Blume
...
function rewriteLink(
  ...
  if (slug !== null) {
    ...
    // Blume base-prefixes site-relative body links to .mdx routes at render
    // time — EXCEPT self-links (it leaves them unprefixed, which 404s under a
...
/**
 * Move the first body H1 into frontmatter `title` and drop the body H1 so Blume
 * renders a single chrome title (it derives the page title + sidebar label from
