<script lang="ts">
  ...
  const childStack = group.stack;
  const childResult = writable<boolean | null>(null);
...
<div role="dialog" aria-modal="true" aria-label={payload.title}>
  ...
  {/if}
  {#each $childStack as raw (raw.id)}
    {@const s = raw as ChildState}
    {@const childCall = callFor(client, group.stackId, raw) as ChildCall | null}
    {#if childCall}
