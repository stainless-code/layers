...
description: Layers repo hybrid deltas for .agents/ — tiers, rule-skill pairing, repo exemplars. Use when creating or reviewing rules/skills in this repo. Read writing-great-skills first for skill vocabulary and authoring principles.
---

# Writing agents config (layers hybrid deltas)
...
## Attachment decision tree (layers)
...
- **Tier-2 attach** — `agents-tier-system` (`.agents/rules/**`, `.agents/skills/**`, `.cursor/rules/**`, `.cursor/skills/**`); `docs-governance-priming` (`docs/**`, `.agents/**`, `.cursor/**`).
...
- ❌ External-monorepo artifact leakage in examples — use `packages/core/src/`, `packages/<fw>/src/`, the package-level core/adapter seam, `bun` scripts, `origin/main`
