...
## § Preserve existing source comments

Non-negotiable when editing source:
...
User-requested doc audits may slim redundant markdown; this preservation applies to **source** comments only.
...
**Keep:** why (not what), non-obvious constraints, cross-cutting context, pointers when a relationship isn't obvious, sentinels/magic values, the rejected alternative, runtime quirks (snapshot referential-stability across `notifyManager.batch`, `AbortController` cancel on dismiss, `gcTime` cache restore).
...
**Historical traces** in committed prose — "as discussed on …", "following up on …", changelog-edit residue, stale rosters — earn no ROI once the moment passes; write as if fresh, cut the trace. Source comments: the rule's "update if outdated" covers them.

**Doc slimming:** full checklists in [`docs-governance/LIFECYCLE.md`](../docs-governance/LIFECYCLE.md) § Existence test + § Cross-reference preservation (or [`docs-lifecycle-sweep`](../docs-lifecycle-sweep/SKILL.md) for a full janitor pass).
...
Before completing a turn that touched code or docs: search the diff for brittle anchors; cut duplicated tables and narration. After a doc slim, confirm runtime invariants (snapshot stability, `gcTime` default, `scope` queue contract) and public-API surface still have a home ([`docs-governance/LIFECYCLE.md`](../docs-governance/LIFECYCLE.md) § Existence test + § Cross-reference preservation).
