...
| "This is a memory leak / race / re-render trap" | Trace the path (`notifyManager.batch` flush, `AbortController` cancel on dismiss, `gcTime` cache teardown, `useSyncExternalStore` unmount-detach). Ask for the scenario.                                                                                   |
...
| "Performance issue"                             | Quantify if possible. Many "performance" comments are speculative — ask for a measurement (snapshot rebuild, scope-queue drain, `gcTime` cache hit/miss).                                                                                                  |
| "Public API should change"                      | Check `packages/*/package.json` `exports` + the package's `src/index.*` exports + adapter JSDoc (and/or `docs/architecture.md` § Type defaults & inference). `createCallContext`'s generic surface and private `Layer`/`LayerStack` fields are deliberate. |
...
| --- | ---------------------------------- | -------------- | --------------------- |
| 1   | packages/core/src/layerStack.ts:42 | …              | Apply suggested diff. |

## ❌ Incorrect / hallucinated (N) — push back

| #   | File:line                      | Claim                             | Why wrong                                                                              |
| --- | ------------------------------ | --------------------------------- | -------------------------------------------------------------------------------------- |
| 2   | packages/solid/src/index.ts:13 | "effect should re-run on stackId" | stackId is captured in the reactive binding; verify whether re-subscription is needed. |
