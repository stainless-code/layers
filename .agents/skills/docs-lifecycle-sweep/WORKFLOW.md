...
**Done when:** every doc file is mapped to a lifecycle type; files fitting none are flagged as rogue.
...
2. **Documents durable policy or framework** unavailable elsewhere (rejected alternatives, decisions of record, the deliberate `gcTime` opt-in, the snapshot referential-stability invariant).
...
rg -n "NOTE\(<topic>" packages/*/src/ scripts/ # marker convention
```
...
3. **Reusable methodology / playbook** that doesn't already live in a skill — verification recipes, adapter-binding test matrices?

**Done when:** each file has recorded cite-check evidence for ≥1 keep-criterion (or none found).
...
| **C — Delete + lift** | Closed AND no source cites AND all findings shipped or lifted to roadmap; OR superseded by a later doc that incorporated everything; OR fails the existence test outright | Lift any orphan-able knowledge into the natural reference doc (`architecture.md`, `glossary.md`, `roadmap.md`, or a skill); **delete the file** (no tombstones)               |

**Done when:** every file carries a Tier A/B/C verdict with its action.
...
| `docs/audits/<rolling-topic>.md`      | Audit    | **A**            | "§-numbers referenced from source — do not renumber" header; M source-cites by §-anchor from `packages/*/src/`                                    | Keep verbatim                                                                                                                         |
...
| `docs/plans/<topic>.md`               | Plan     | **C**            | Work shipped on commits X, Y; lifted destinations confirmed in `architecture.md`                                                                  | Delete after lifting any remaining open work to `roadmap.md`                                                                          |
...
**Done when:** the classification table — with diff previews and cross-reference impact — is shown to the user before any edit.
...
4. **Re-grep** to confirm zero broken cross-references: `rg "<deleted-filename>"` returns 0 hits outside the deletion commit message.

**Done when:** approved actions are applied in dependency order and a re-grep confirms zero broken cross-references.
...
If the user wants a durable record (e.g. "we deleted N audits, here's why"), lift only non-reconstructable rationale into its natural permanent home (`architecture.md`, a rule, or a skill). Keep the forward-looking `roadmap.md` for open work. Default is: don't write a meta-doc about the cleanup.
