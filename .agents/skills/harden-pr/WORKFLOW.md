...
| **Structure**   | No boundary violations in the diff (core stays zero-dep; adapter entries own their peer; no cycles)                                                                                                                         |
...
```text
You are the **{ROLE}** reviewer for `/harden-pr` on `@stainless-code/layers`.
...
3. **Structure (lite)** — diff imports for boundary violations (core zero-dep gate, adapter-package peer ownership, no cross-adapter imports).
...
| **Performance** | Hot paths in the diff (snapshot rebuild, scope-queue drain, `gcTime` cache, batch flush)         |
