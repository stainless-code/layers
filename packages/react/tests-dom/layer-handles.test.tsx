...
describe("React adapter — layer handles", () => {
  ...
  it("useLayer upsert merges into the topmost same-key layer", async () => {
    const client = new LayerClient();
    let handle!: WiredLayerHandle<{ msg: string }, void>;
    ...
    await waitFor(() =>
      expect(screen.getByTestId("top-msg").textContent).toBe("b"),
    );
    expect(screen.getByTestId("state-len").textContent).toBe("1");
    await handle.dismiss(undefined as void);
  });
