...
- **Runtimes** — **Node** `^20.19.0 || >=22.12.0` and **Bun** `>=1.4.0` at the private monorepo root (`packageManager: bun@1.4.1`). Published adapter packages keep `engines.bun` `>=1.0.0`. The core is zero-dep by design; each adapter package declares its required peer (`react`, `preact`, `solid-js`, `@angular/core`, `vue`, `lit` + `@lit/context`, `svelte`).
...
3. Install on `stainless-code/layers`.
